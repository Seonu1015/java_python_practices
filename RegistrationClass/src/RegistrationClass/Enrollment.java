...
public class Enrollment implements InterfaceEnroll {
	...
	private static void listAllCourse(ArrayList<CourseInfo> availableCourse) {
		...
		for (int i = 0; i < availableCourse.size(); i++) {
			...
			System.out
					.println(i + 1 + ". " + course.getCourseName() + " (교수: " + course.getProfessor().getName() + ")");
		}
	}
	...
	private void listEnrolledCourse() {
		if (enrolledCourse.isEmpty()) {
			...
			for (int i = 0; i < enrolledCourse.size(); i++) {
				...
				System.out.println(
						i + 1 + ". " + course.getCourseName() + " (교수: " + course.getProfessor().getName() + ")");
			}
		}
	}
	...
	public void enrollCourse() {
		...
		CourseInfo selectedCourse = availableCourse.get(selectCourseNum-1);
	...
	public void dropCourse() {
	    ...
	    if (courseDelete >= 1 && courseDelete <= enrolledCourse.size()) {
	    	CourseInfo droppedCourse = enrolledCourse.get(courseDelete - 1);
	        String deletedCourseName = droppedCourse.getCourseName();
	        droppedCourse.dropStudent(student);
	        enrolledCourse.remove(courseDelete - 1);
